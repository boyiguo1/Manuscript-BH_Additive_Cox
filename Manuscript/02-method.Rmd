...
\end{equation}
Leveraging the relationship between double exponential prior and LASSO, the product of partial likelihood function and the prior of $\bs \beta$ can be viewed as an $l_1$ penalized partial likelihood function with penalty xxx for $\beta_j$ and $\beta_{jk}$ and optimized with the coordinate descent algorithm [TODO: add citation]. Nevertheless, the optimization requires the knowledge of $\bs \gamma$, which is unknown. To address the problem, we treat the latent indicators $\bs \gamma$ as the "missing data" and use EM algorithm to derive the MAP estimates. Notably, [TO explain the algorithm]


[TODO: revise after this]

It incorporates the IRLS algorithm into the EM steps to iteratively find the MAP estimates. Thanks to the versatility of IRLS algorithm, the proposed algorithm can dealt with many family of outcomes. Also, the proposed can work not only for the prior previously mentioned, but also some modified priors, for example mixed double exponential.
